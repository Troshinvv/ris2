...
void v1_proton_centrality(){
  ...
  auto ratio_builder = RatioBuilder<Correlation>{
    ...
    []( auto& container ){ 
      container = container.Rebin( {"centrality", 1, 10, 30} ); 
    }
  };
  ratio_builder
  .AddToBunch(
    ...
    []( auto& container ){ 
      container = container.Rebin( {"centrality", 1, 5, 25} ); 
    }
  )
  .AddToBunch(
    "15-35%"s,
    ...
    []( auto& container ){ 
      container = container.Rebin( {"centrality", 1, 15, 35} ); 
    }
  )
  ;
  ...
  ratio_builder.Perform([]( auto& obj ){
    obj
    .Rebin( std::vector<Qn::AxisD>{ { "trPt", 1, 0.8, 1.6 } } )
  ...
  plot.Print( "/home/mikhail/ris2/macro/bmn_run8/analysis_note/v1/pictures/v1_proton_centrality.png" );
}