...
void v1_proton_nhits(){
  gROOT->Macro( "/home/mikhail/ris2/macro/bmn_run8/analysis_note/style.cc" );
  auto file_100 = std::string{"/home/mikhail/ris2/macro/bmn_run8/analysis_note/files/vf.recent.nhits.2024.06.15.root"};
  ...
  auto ratio_builder = RatioBuilder<Correlation>{
    ...
    std::vector<std::string>{
      "proton_nhits_5/v1.F1_RESCALED(F2_RESCALED,F3_RESCALED).y1y1centrality",
      "proton_nhits_5/v1.F2_RESCALED(F1_RESCALED,F3_RESCALED).y1y1centrality",
      "proton_nhits_5/v1.F3_RESCALED(F1_RESCALED,F2_RESCALED).y1y1centrality",
    }
  };
  ratio_builder
  .AddToBunch(
    ...
    file_100, 
    std::vector<std::string>{
      "proton_nhits_4/v1.F1_RESCALED(F2_RESCALED,F3_RESCALED).y1y1centrality",
      "proton_nhits_4/v1.F2_RESCALED(F1_RESCALED,F3_RESCALED).y1y1centrality",
      "proton_nhits_4/v1.F3_RESCALED(F1_RESCALED,F2_RESCALED).y1y1centrality",
    }
  )
  .AddToBunch(
    "N_{hits} > 6"s,
    file_100, 
    std::vector<std::string>{
      "proton_nhits_6/v1.F1_RESCALED(F2_RESCALED,F3_RESCALED).y1y1centrality",
      "proton_nhits_6/v1.F2_RESCALED(F1_RESCALED,F3_RESCALED).y1y1centrality",
      "proton_nhits_6/v1.F3_RESCALED(F1_RESCALED,F2_RESCALED).y1y1centrality",
    }
  )
  ;
  ...
  y_proj_sys.Perform( []( auto graph ){
    for( int i=0; i<graph->GetN(); ++i ){
      auto y_err = graph->GetPointY(i) * 0.05;
  ...
  //   .GetSubPlot(1)
  //   .SetXAxis(Axis().SetTitle("y_{cm}").SetLo(-0.2).SetHi(1.0))
  //   .SetYAxis(Axis().SetTitle("ratio").SetLo(0.8).SetHi(1.2))
  // ;

  plot.Print( "/home/mikhail/ris2/macro/bmn_run8/analysis_note/v1/pictures/v1_proton_nhits.png" );
}