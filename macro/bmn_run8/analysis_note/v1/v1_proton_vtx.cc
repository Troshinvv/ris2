...
void v1_proton_vtx(){
  ...
  auto file_100 = std::string{"/home/mikhail/ris2/macro/bmn_run8/analysis_note/files/vf.recent.vtx.2024.06.17.root"};
  ...
  auto ratio_builder = RatioBuilder<Correlation>{
    ...
    std::vector<std::string>{
      "proton/v1.F1_RESCALED(F2_RESCALED,F3_RESCALED).y1y1centralityvtxXvtxY",
      "proton/v1.F2_RESCALED(F1_RESCALED,F3_RESCALED).y1y1centralityvtxXvtxY",
      "proton/v1.F3_RESCALED(F1_RESCALED,F2_RESCALED).y1y1centralityvtxXvtxY",
    }
  };
  ratio_builder
  .AddToBunch(
    ...
    []( auto& container ){ 
      container = container.Rebin( {"vtxX", 1, 0, 1} ); 
      container = container.Rebin( {"vtxY", 1, 0, 1} ); 
    }
  )
  .AddToBunch(
    "90-180"s,
    ...
    []( auto& container ){ 
      container = container.Rebin( {"vtxX", 1, -1, 0} ); 
      container = container.Rebin( {"vtxY", 1, 0, 1} ); 
    }
  )
  .AddToBunch(
    ...
    []( auto& container ){ 
      container = container.Rebin( {"vtxX", 1, -1, 0} ); 
      container = container.Rebin( {"vtxY", 1, -1, 0} ); 
    }
  )
  .AddToBunch(
    "270-360"s,
  ...
  plot.Print( "/home/mikhail/ris2/macro/bmn_run8/analysis_note/v1/pictures/v1_proton_vtx.png" );
}