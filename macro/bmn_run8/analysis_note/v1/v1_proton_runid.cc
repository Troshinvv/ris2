...
void v1_proton_runid(){
  gROOT->Macro( "/home/mikhail/ris2/macro/bmn_run8/analysis_note/style.cc" );
  auto file_100 = std::string{"/home/mikhail/ris2/macro/bmn_run8/analysis_note/files/vf.recent.runid.2024.06.17.root"};
  ...
  auto ratio_builder = RatioBuilder<Correlation>{
    ...
    std::vector<std::string>{
      "proton/v1.F1_RESCALED(F2_RESCALED,F3_RESCALED).y1y1centralityrunId",
      "proton/v1.F2_RESCALED(F1_RESCALED,F3_RESCALED).y1y1centralityrunId",
      "proton/v1.F3_RESCALED(F1_RESCALED,F2_RESCALED).y1y1centralityrunId",
    }
  };
  ratio_builder
  .AddToBunch(
    ...
    []( auto& container ){ 
      container = container.Rebin( {"runId", 1, 7000, 7300} ); 
    }
  )
  .AddToBunch(
    "7300-7600"s,
    ...
    []( auto& container ){ 
      container = container.Rebin( {"runId", 1, 7300, 7600} ); 
    }
  )
  .AddToBunch(
    "7600-7900"s,
    ...
    []( auto& container ){ 
      container = container.Rebin( {"runId", 1, 7600, 7900} ); 
    }
  )
  .AddToBunch(
    "7900-8200"s,
    ...
    []( auto& container ){ 
      container = container.Rebin( {"runId", 1, 7900, 8200} ); 
    }
  )
  ;
  ...
  ratio_builder.SetPalette(
    {
      ...
      Style().SetColor(kYellow+2).SetMarker(kFullTriangleUp),
      Style().SetColor(kCyan+2).SetMarker(kFullDiamond),
    }
  );
  ...
  y_proj_sys.Perform( []( auto graph ){
    for( int i=0; i<graph->GetN(); ++i ){
      auto y_err = graph->GetPointY(i) * 0.06;
  ...
  plot.Print( "/home/mikhail/ris2/macro/bmn_run8/analysis_note/v1/pictures/v1_proton_runid.png" );
}